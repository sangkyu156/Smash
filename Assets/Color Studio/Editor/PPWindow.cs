    ...
    public partial class PPWindow : EditorWindow {
        ...
        void OnEnable() {
            ...
            brushIcons = new GUIContent[] {
                new GUIContent("Pen", "단일 픽셀을 칠합니다."),
                new GUIContent("지우개", "투명한 색상으로 페인트"),
                new GUIContent("바꾸다", "동일한 색상의 모든 픽셀을 대체합니다."),
                new GUIContent("홍수(꽉) 채우기", "인접한 영역을 동일한 색상으로 채웁니다."),
                new GUIContent("어둡게", "픽셀을 더 어둡게 만듭니다."),
                new GUIContent("밝게", "픽셀을 더 밝게 만듭니다."),
                new GUIContent("Dry", "픽셀의 채도를 낮춥니다."),
                new GUIContent("Vivid(선명한)", "픽셀을 더욱 다채롭게 만듭니다."),
                new GUIContent("Noise", "무작위로 픽셀을 더 어둡게 또는 더 밝게 만듭니다."),
                new GUIContent("Noise Tone", "픽셀 톤을 무작위로 변경합니다."),
                new GUIContent("Gradient(경사도)", "픽셀을 칠할 때 더 어두운 색상으로 전환")
            };
            ...
            brushShapes = new GUIContent[] {
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "shape_solid"), "입체형상"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "shape_circle"), "써클형태"),
            ...
            icons = new GUIContent[] {
                new GUIContent("", "Color Studio 팔레트 관리자를 엽니다. 픽셀을 마우스 오른쪽 버튼으로 클릭하여 해당 색상을 선택할 수도 있습니다."),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "save"), "새 텍스처 파일을 대체하거나 생성합니다."),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "save"), "새로운 스프라이트 파일을 대체하거나 생성합니다."),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "transparent"), "투명한 색상을 선택합니다. 픽셀을 마우스 오른쪽 버튼으로 클릭하여 해당 색상을 선택할 수도 있습니다."),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "undo"), "마지막 텍스처 작업 실행 취소"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "redo"), "마지막 텍스처 작업 다시 실행"),

                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_flip_vert"), "이미지를 수직으로 뒤집기"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_flip_horiz"), "이미지를 수평으로 뒤집기"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_disp_left"), "이미지를 왼쪽으로 이동"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_disp_right"), "이미지를 오른쪽으로 이동"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_disp_up"), "이미지를 위로 이동"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_disp_down"), "이미지를 아래로 이동"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_rot_left"), "이미지를 90° CCW 회전"),
                new GUIContent(Resources.Load<Texture2D>(iconsPath + "cmd_rot_right"), "이미지를 시계 방향으로 90° 회전")
        };
            mirrorModes = new GUIContent[] {
                new GUIContent("None", "미러링 없음"),
                new GUIContent("Horiz", "수평으로 미러링"),
                new GUIContent("Vertical", "수직으로 미러링"),
                new GUIContent("Quad", "수평 및 수직 모두 미러링")
            };
        ...
        void DrawCanvasArea() {
            ...
            if (GUILayout.Button("Set Size", GUILayout.Height(32))) {
            ...
            EditorGUILayout.EndHorizontal();
            if (GUILayout.Button("Fit Palette", GUILayout.Height(32))) {
                FitPalette();
            }
        ...
        void UpdateCanvasMaterialProperties() {
            ...
            float pixelOffset = currentBrush.usesBrush() ? (brushPixelWidth % 2) * 0.5f : 0.5f;
            ...
            canvasMaterial.SetFloat("_GridWidth", _showGrid ? 0.002f * zoomFactor : 0);
            ...
            canvasMaterial.SetVector("_TileSize", new Vector4(1f / (int)(width * zoomFactor), 1f / (int)(height * zoomFactor), zoomFactor, canShowGrid ? 1: 0));
            canvasMaterial.SetVector("_ZoomRect", new Vector4(uvXmin, uvYmin, uvXmax, uvYmax));
        }
